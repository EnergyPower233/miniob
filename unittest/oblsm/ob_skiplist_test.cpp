...
uint32_t murmurhash(const char* data, size_t n, uint32_t seed)
{
  ...
  switch (limit - data) {
    case 3: h += static_cast<uint32_t>(static_cast<int8_t>(data[2])) << 16;
    case 2: h += static_cast<uint32_t>(static_cast<int8_t>(data[1])) << 8;
    case 1:
...
public:
  ...
  explicit TestState() : quit_flag_(false), state_(STARTING), pending_writers_(0), state_cv_() {}
